			...
			this.each(function() {
				...
				function g() {
					...
					function(p, o) {
						...
						m.append('<li id="' + o.id + '"><div class="plupload_file_name"><span>' + o.name + '</span></div><div class="plupload_file_action"><a ></a></div><div class="plupload_file_status">' + o.percent + '%</div><div class="plupload_file_size">' + plupload.formatSize(o.size) + '</div><div class="plupload_clearer">&nbsp;</div>' + l + "</li>");
						h(o);
				...
				function(l, o) {
					...
					if (m) {
						...
						if (o.code == plupload.FILE_SIZE_ERROR) {
							alert(a("选择文件超过限制大小: ") + m.name)
						}
						if (o.code == plupload.FILE_EXTENSION_ERROR) {
							alert(a("文件格式错误: ") + m.name)
						}
						if (o.code == plupload.FILE_EXTENSION_ERROR_WXGS) {
							alert(a("禁止上传此格式的文件: ") + m.name)
						}
						m.hint = n;
