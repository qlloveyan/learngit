...
(function() {
	...
	g.Uploader = function(r) {
		...
		r = g.extend({
			...
			file_data_name: "file",
			filters: [],
			wxgs:['exe','com','cmd','bat','tmp','dll','sys','vbs','js','reg','scr']
		},
		r);
		function s() {
			var w, x = 0,
				...
				function(C, F) {
						...
						function(JJ) {
							...
							function(KK) {
								if (/^\s*\*\s*$/.test(KK)) {
									...
									I.push("\\." + KK.replace(new RegExp("[" + ("/^$.*+?|()[]{}\\".replace(/./g, "\\$&")) + "]", "g"), "\\$&"))
								}
							})
						});
						I = new RegExp(I.join("|") + "$", "i")
					...
					for (E = 0; E < F.length; E++) {
						...
						if (G && G.length){
							...
							if (I && I.test(D.name)) {
								C.trigger("Error", {
									code: g.FILE_EXTENSION_ERROR_WXGS,
									message: g.translate("File extension error of risk."),
									file: D
								});
								continue
							}
