	...
	function create(target){
		...
		var win = $(target).panel($.extend({}, state.options, {
			...
			onOpen: function(){
				...
				state.window.css('z-index', state.options.zIndex++);
	...
	function setProperties(target){
		...
		state.window.draggable({
			...
			onStartDrag: function(e){
				if (state.mask) state.mask.css('z-index', state.options.zIndex++);
				if (state.shadow) state.shadow.css('z-index', state.options.zIndex++);
				state.window.css('z-index', state.options.zIndex++);
