Surfilter.ns("main",{
	...
	initMenu : function(){
		...
		$(Surfilter.menus).each(function(firstLevelIndex,firstLevelMenu){//遍历菜单数组
			$('<li class="active"><a href="javascript:void()">'+ firstLevelMenu.text +'</a></li>').appendTo('.navbar-inner ul.nav').click(function(){
				// 显示二级菜单
				...
				$(Surfilter.menus[firstLevelIndex].children).each(function(secondLevelIndex,secondLevelMenu){
					$secondMenu = $('<li><a href="javascript:void()"><i class="ico ' + secondLevelMenu.iconCls + '"></i>' + secondLevelMenu.text + '</a></li>');
					...
					if(secondLevelMenu.children.length > 0){ 
						...
						$.each(secondLevelMenu.children,function(thirdLevelIndex,thirdLevelMenu){
							$('<li><a href="javascript:void()"><i class="ico ' + thirdLevelMenu.iconCls + '"></i>' + thirdLevelMenu.text + '</a></li>').appendTo($thirdMenu).click(function(event){
	 ...
	 */
	changPass:function(){
		$("#changePasDlg").css("z-index","1050");
		$('#changePasDlg').modal('show');
	},
	 ...
	 */
	changeSure:function(){
		var flag = $("#changeForm").form('validate');
		if( flag ){
			var oldPas = $("#changeForm").find("input[name='oldPas']").val();
			var password = $("#changeForm").find("input[name='password']").val();
			$.ajax({
				url : ctx +  "/user/changePassword.do",
				async : false,
				datatype:'json',
				data:{'password':oldPas,'newPassword':password},
				success : function(data){
					var res = $.parseJSON(data);
					Msgslide(res.msg);
					if( res.success ){
						$('#changePasDlg').modal('hide');
						$("#changePasDlg").css("z-index","-1");
					}
				}
			});
		}
